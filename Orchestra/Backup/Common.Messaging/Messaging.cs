...
namespace Common.Messaging
{
    ...
    public static class Messaging
    {
        ...
        internal class ZeroMQBase : IConsume, IPublish, IMessaging
        {
            ...
            /// The _connection
            /// </summary>
            private static Context _zmqContext;
            /// <summary>
            /// The channel
            /// </summary>
            private Socket _zmqSocket;
            /// <summary>
            /// The _type
            /// </summary>
            ...
            public IConsume Consumer
            {
                get
                {
                    ...
                    {
                        case Topology.Basic:
                            _zmqSocket = _zmqContext.Socket(SocketType.PULL);
                            break;
                        //case Topology.PubSub:
            ...
            public IPublish Publisher
            {
                get
                {
                    ...
                    {
                        case Topology.Basic:
                            _zmqSocket = _zmqContext.Socket(SocketType.PUSH);
                            break;
                        //case Topology.PubSub:
            ...
            protected ZeroMQBase(Topology topology, string bindings)
            {
                ...
                if (_zmqContext == null) _zmqContext = new Context(1);
            }
            ...
            public void DeQueue(bool purge)
            {
                ...
                Action startDequeue = () =>
                {
                    //var pollItem = new[] {_zmqSocket.CreatePollItem(IOMultiPlex.POLLIN)};
                    //_zmqSocket.PollInHandler += _zmqSocket_PollInHandler;

                    while (Interlocked.Read(ref _continueDequeue) != 0)
                    {
                        try
                        {
                            var data = _zmqSocket.Recv();
                            if (data != null) HandleMessage(FromByte(data));
                        }
            ...
            void _zmqSocket_PollInHandler(Socket socket, IOMultiPlex revents)
            {
                HandleMessage(FromByte(socket.Recv()));
            }
